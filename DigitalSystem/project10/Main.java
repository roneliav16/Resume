...
public class Main {
    ...
    public static void processFiles(File f) throws IOException {
        ...
        if (f.isDirectory()) {
            ...
            JackAnalyzer.processMultiple(f);
        }
        ...
        if (f.isFile()) {
            ...
            String fileMameWithoutExtending = f.getName().substring(0, f.getName().lastIndexOf('.')) + ".xml";
            ...
            FileWriter fileWriter = new FileWriter(out_asm, false);

            // Wrap the FileWriter in a BufferedWriter to improve efficiency
            BufferedWriter writer = new BufferedWriter(fileWriter);

            // Process the input file and write its content to the .xml file
            JackAnalyzer.processOneFile(f, writer);
